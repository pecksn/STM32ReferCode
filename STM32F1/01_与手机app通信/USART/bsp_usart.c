...
void USART1_IRQHandler(void)
{
    ...
    static uint8_t  RxTemp[U1_RX_BUF_SIZE];                          // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断), 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；
    ...
    if (USART1->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        memcpy(xUSART.USART1ReceivedData, RxTemp, U1_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中, 等待处理; 注意：复制的是整个数组，包括0值，以方便字符串数据
...
void USART2_IRQHandler(void)
{
    ...
    if (USART2->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        memcpy(xUSART.USART2ReceivedData, RxTemp, U2_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中, 等待处理; 注意：复制的是整个数组，包括0值，以方便字符串数据
...
uint8_t USART2_GetBuffer(uint8_t *buffer, uint8_t *cnt)
{
    ...
    {
        memcpy(buffer, xUSART.USART2ReceivedData, xUSART.USART2ReceivedNum);   // 把新数据复制到指定位置
...
{
    static uint16_t cnt = 0;                                         // 接收字节数累计：每一帧数据已接收到的字节数
    static uint8_t  RxTemp[U3_RX_BUF_SIZE];                          // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断), 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；
    ...
    if (USART3->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        memcpy(xUSART.USART3ReceivedData, RxTemp, U3_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中, 等待处理; 注意：复制的是整个数组，包括0值，以方便字符串数据
...
uint8_t USART3_GetBuffer(uint8_t *buffer, uint8_t *cnt)
{
    ...
    {
        memcpy(buffer, xUSART.USART3ReceivedData, xUSART.USART3ReceivedNum);   // 把新数据复制到指定位置
...
{
    static uint16_t cnt = 0;                                        // 接收字节数累计：每一帧数据已接收到的字节数
    static uint8_t  RxTemp[U4_RX_BUF_SIZE];                         // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断), 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；
    ...
    if (UART4->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        memcpy(xUSART.UART4ReceivedData, RxTemp, U4_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中, 等待处理; 注意：复制的是整个数组，包括0值，以方便字符串数据
...
uint8_t UART4_GetBuffer(uint8_t *buffer, uint8_t *cnt)
{
    ...
    {
        memcpy(buffer, xUSART.UART4ReceivedData, xUSART.UART4ReceivedNum); // 把新数据复制到指定位置
...
{
    static uint16_t cnt = 0;                                        // 接收字节数累计：每一帧数据已接收到的字节数
    static uint8_t  RxTemp[U5_RX_BUF_SIZE];                         // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断), 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；
    ...
    if (UART5->SR & (1 << 4))                                       // 检查IDLE(空闲中断标志位); IDLE中断标志清理方法：序列清零，USART1 ->SR;  USART1 ->DR;
    {
        ...
        memcpy(xUSART.UART5ReceivedData, RxTemp, U5_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中, 等待处理; 注意：复制的是整个数组，包括0值，以方便字符串数据
...
uint8_t UART5_GetBuffer(uint8_t *buffer, uint8_t *cnt)
{
    ...
    {
        memcpy(buffer, xUSART.UART5ReceivedData, xUSART.UART5ReceivedNum);   // 把新数据复制到指定位置
